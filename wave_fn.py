...
Takes array of 'sst', a time frame array (in datetime format),
a time step and a title to plot on figure

'''

def wave_fn(sst, time_frame, dt, title = 'Test', cmapp = 'Greys'):
	...
	plt.title(str(time_frame[0])[0:19]+ ' Modified wavelet ' + title)
	plt.hold(False)
	...
	plt.xlabel('Start time ('+ str(time_frame[0])[0:19]+')')
	plt.ylabel('Wavelet Time Scale (seconds)')
	...
	xx = np.arange(0, int(dt)*n, 1500)
	xx = np.array(xx)
	time_arrayy = time_frame[xx/int(dt)]
	for i in range(len(time_arrayy)):
		time_arrayy[i] = str(time_arrayy[i])[11:16]



	plt.xticks(xx, time_arrayy)
